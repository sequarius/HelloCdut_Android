...
public class GroupBlacklistActivity extends BaseActivity {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            CommonUtils.showCustomToast(Toast.makeText(getApplicationContext(), "获取失败,请检查网络或稍后再试", 1));
                            progressBar.setVisibility(View.INVISIBLE);
                        }
                    });
                }
    ...
    void removeOutBlacklist(final String tobeRemoveUser) {
        try {
            ...
            runOnUiThread(new Runnable() {
                public void run() {
                    CommonUtils.showCustomToast(Toast.makeText(getApplicationContext(), "移出失败", 0));
                }
            });
        }
    }
