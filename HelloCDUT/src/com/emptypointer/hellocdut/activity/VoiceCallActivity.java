...
public class VoiceCallActivity extends Activity implements OnClickListener {
        ...
        if (!isInComingCall) {// 拨打电话
            ...
            try {
                ...
                runOnUiThread(new Runnable() {
                    public void run() {
                        CommonUtils.showCustomToast(Toast.makeText(VoiceCallActivity.this, "尚未连接至服务器", 0));
                    }
                });
            }
        } else { // 有电话进来
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.iv_mute: // 静音开关
                if (isMuteState) {
                    // 关闭静音
                    muteImage.setImageResource(R.drawable.ic_mute_normal);
                    audioManager.setMicrophoneMute(false);
                    isMuteState = false;
                } else {
                    // 打开静音
                    muteImage.setImageResource(R.drawable.ic_mute_on);
            ...
            case R.id.iv_handsfree: // 免提开关
                if (isHandsfreeState) {
                    // 关闭免提
                    handsFreeImage.setImageResource(R.drawable.ic_speaker_normal);
                    closeSpeakerOn();
                    isHandsfreeState = false;
                } else {
                    handsFreeImage.setImageResource(R.drawable.ic_speaker_on);
                    openSpeakerOn();
    ...
    private void saveCallRecord() {
        ...
        // 设置扩展属性
        message.setAttribute(GlobalVariables.MESSAGE_ATTR_IS_VOICE_CALL, true);
